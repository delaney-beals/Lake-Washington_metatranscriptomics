...
gbk_dir <- "C:/Users/Delaney/OneDrive/unknown/Documents/antismash_download/Antismash_107_bins_HTML_report/Antismash on collection 107_ HTML report/KBase_derived_bin.050.fastanoDAS_assembly.RAST.gbff/"

# Read the index.html file and extract sequence names
index_file <- "C:/Users/Delaney/OneDrive/unknown/Documents/antismash_download/Antismash_107_bins_HTML_report/Antismash on collection 107_ HTML report/KBase_derived_bin.050.fastanoDAS_assembly.RAST.gbff/index.html"
...
for (i in seq_along(gbk_files)) {
  ...
  output_bam <- paste0("LWdata/21113X1_accepted_hits_", seqname, "_BGC.bam")
...
# Keep only rows containing "bin.050" in the "file_name" column
...
NodesCountsRegion <- as.data.frame(inner_join(summarized_counts, nodeAndRegionData_050, by = "Node"))

NodesCountsRegionMetab <- inner_join(NodesCountsRegion, secondary_metabolites_050, by = "Region_ID")
...
## Filter the dataframe to keep only rows where 'bin' contains 'bin.050'
filtered_data_050 <- aggregated_data %>% 
  filter(grepl("bin\\.050", bin))

# Aggregate the read data 
aggregated_reads_050 <- NodesCountsRegionMetab %>%
  group_by(Type) %>%
  summarise(total_counts = sum(counts, na.rm = TRUE))

# Join the tables on the "Type" column
joined_data <- inner_join(aggregated_reads_050, filtered_data_050, by = "Type")
